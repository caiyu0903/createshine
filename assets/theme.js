...
(function (bodyScrollLock, themeAddresses, themeCurrency, Rellax, Flickity, FlickityFade, MicroModal, themeImages) {
  ...
  */
    ...
    const selectors$U = {
        ...
        newCartTotal: '[data-new-cart-total]',
        ...
        newNavLink: '[data-new-nav-link]',
        newNavItemCart2: '[data-new-nav-item-cart2]',
        newNavProduct2: '[data-new-nav-product2]',
        newNavLink2: '[data-new-nav-link2]',
        newCartCheckout: '[data-new-cart-checkout]',
    ...
    class CartDrawer {
        ...
        init() {
            ...
            this.newNavItemCart = document.querySelector(selectors$U.newNavItemCart);
            this.newNavProduct = document.querySelector(selectors$U.newNavProduct);
            this.newNavLink = document.querySelector(selectors$U.newNavLink);
            this.newNavItemCart2 = document.querySelector(selectors$U.newNavItemCart2);
            this.newNavProduct2 = document.querySelector(selectors$U.newNavProduct2);
            this.newNavLink2 = document.querySelector(selectors$U.newNavLink2);
            this.newCartCheckout = document.querySelector(selectors$U.newCartCheckout);
            this.items = document.querySelectorAll(selectors$U.item);
            this.cartTotal = document.querySelector(selectors$U.cartTotal);
            this.newCartTotal = document.querySelector(selectors$U.newCartTotal);
            ...
            // 定义购物车对象取决于我们是否有购物车抽屉或购物车页面
            this.cart = this.cartDrawer || this.cartPage;

            this.form = null;

            this.build = this.build.bind(this);
            this.builds = this.builds.bind(this);
            this.newCheckout = this.newCheckout.bind(this);
            this.readProduct();
            this.newCartEvents();

            // AJAX request
            this.addToCarts = this.addToCarts.bind(this);
        ...
        addToCartCallback(event) {
            ...
            this.addToCarts(formData, quickAddObject, button);
            ...
            cartItemRemove.forEach((button) => {
                ...
                button.addEventListener('click', (event) => {
                    ...
                    if (button1) {
                        ...
                        this.newNavProduct.classList.remove('active')
                        $.ajax({
                            type: "GET",
                            url: window.location.href,
                            success: function (data) {
        ...
        cartEventAdd() {
            document.addEventListener('click', (event) => {
                ...
                if (clickedElement.matches(selectors$U.newNavCart) || (clickedElement.closest(selectors$U.newNavCart) && clickedElement)) {
                    let formData = '';
                    let button = clickedElement.matches(selectors$U.newNavCart) ? clickedElement : clickedElement.closest(selectors$U.newNavCart);
                    ...
                    this.addToCarts(formData, null, button);
         ...
         * @return  {Void}
         */
        addToCarts(formData, quickAddObject = null, button = null) {
            if (this.cart) {
        ...
        builds(data) {
            ...
            const cartItems = data.querySelector(selectors$U.item);
            ...
            if (cartEmptyData) {
                ...
                if (this.newNavItem) {
                    ...
                    if (location.includes('/collections/all-settings/product')) {
                        if (this.newNavLink) {
                            console.log(this.newNavLink)
                            // this.newNavProduct.removeChild(this.newNavLink)
                            this.newNavLink.style.display = 'none'
                        }
                        this.newNavProduct.classList.add('active')
                        this.newNavItemCart.innerHTML = cartItemsData.innerHTML;
                        const newCartItems = this.newNavItemCart.closest(selectors$U.item);
                    ...
                    if (location.includes('/collections/diamonds/products')) {
                        if (this.newNavLink2) {
                            // this.newNavProduct2.removeChild(this.newNavLink2)
                            this.newNavLink2.style.display = 'none'
                        }
            ...
            this.newCartTotal.innerHTML = this.subtotal === 0 ? window.theme.strings.free : themeCurrency.formatMoney(this.subtotal, theme.moneyWithCurrencyFormat);
            window.sessionStorage.setItem('this.newCartTotal.innerHTML', this.newCartTotal.innerHTML);
            console.log(this.cart.classList)
            ...
            // 添加类“is-updated”行项目持有人，通过CSS变量减少购物车项目动画延迟
            if (this.cartDrawerIsOpen) {
        ...
        readProduct() {
            if (window.location.href.includes('/collections/all-settings') || window.location.href.includes('/collections/diamonds')) {

                const product1 = window.sessionStorage.getItem('this.newNavItemCart.innerHTML', this.newNavItemCart.innerHTML)
                const product2 = window.sessionStorage.getItem('this.newNavItemCart2.innerHTML', this.newNavItemCart2.innerHTML)
                const totalPrice1 = window.sessionStorage.getItem('this.newCartTotal.innerHTML', this.newCartTotal.innerHTML)
                if (product1 !== null) {
                    this.newNavLink.style.display = 'none';
                    // this.newNavProduct.removeChild(this.newNavLink)
                    this.newNavProduct.classList.add('active')
                    this.newNavItemCart.innerHTML = product1;
                }
                ...
                if (product2 !== null) {
                    ...
                    // this.newNavProduct2.removeChild(this.newNavLink2)
                    this.newNavProduct2.classList.add('active')
                    this.newNavItemCart2.innerHTML = product2;
                }
        ...
        newCheckout(data) {
            const newCartCheckout = document.querySelector(selectors$U.newCartCheckout);
            console.log(newCartCheckout)
            const cartItemsData = data.querySelector(selectors$U.apiLineItems);
            newCartCheckout.addEventListener('click', (event) => {
                event.preventDefault();
                this.itemsHolder.innerHTML = cartItemsData.innerHTML;
                document.dispatchEvent(
                    new CustomEvent('theme:product:added', {
            ...
            if (skipButtons.length) {
                skipButtons.forEach((button) => {
                    button.addEventListener('click', (event) => {
                        ...
                        // Add skipped upsell product to session storage 将跳过的追加销售产品添加到会话存储
        ...
        observeAdditionalCheckoutButtons() {
            ...
            if (additionalCheckoutButtons) {
                ...
                // 创建一个名为' observer '的' MutationObserver '的新实例
                // passing it a callback function 给它传递一个回调函数
    ...
    if (closeButton) {
        closeButton.addEventListener('click', (e) => {
            ...
            window.sessionStorage.removeItem('this.newCartTotal.innerHTML');
            // 重载当前页面
            e.target.closest('[data-new-nav-product2]').classLise.remove('active')
            e.target.closest('[data-new-nav-product2] ')
            console.log("nihao")
   ...
   */
    ...
    class ProductAddForm {
        ...
        updateAddToCartState(formState) {
            ...
            const navClick = this.container.querySelectorAll(selectors$v.navClick);
    ...
    class Collection {
        constructor(section) {
            ...
            this.accessibility = a11y;
            // 表格价格
            this.new_price = this.container.querySelector('.new-price-sort')
            ...
            this.onSortNewClickEvent = (e) => this.onSortNewClick(e);
        ...
        onSortNewClick(submitForm = true) {
            const link = document.querySelector('.new-price-sort');
            const sort = link.getAttribute('data-value');
            const suffixList = ["?sort_by=price-ascending", "?sort_by=price-descending"];
            let currentSuffixIndex = 0;
            currentSuffixIndex++; // 累加当前后缀索引
            if (currentSuffixIndex >= suffixList.length) {
                currentSuffixIndex = 0; // 执行到最后一个元素，回到第一个元素
            }
            const newSuffix = suffixList[currentSuffixIndex];
            // 获得新的后缀值
            const newUrl = newSuffix.replace(/\?.*$/, '') + 'nihao'; // 把后缀添加到当前URL中，并更新 URL
        ...
        initSort() {
            if (this.new_price) {
                ...
                this.new_price.addEventListener('click', this.onSortNewClickEvent);
            }
